...
@main struct AudioOwnershipSyntheticTests {
    static func main() {
        ...
        precondition(!deviceA.streamConfiguration(showsCursor: false), "fresh authenticated owner starts audio-off")
        deviceA.handleSystemAudioCommand(Data("SNDON".utf8))
        precondition(deviceA.stream?.configurations.last == true)
        precondition(!deviceB.streamConfiguration(showsCursor: false), "another device needs its own opt-in")
        precondition(anotherEndpointA.stream?.configurations.last == false, "nonowner stays audio-off")
        sessions = [:]
        let first = CaptureSession(1, 77), sibling = CaptureSession(2, 77)
        sessions = [1: first, 2: sibling]
        precondition(!first.streamConfiguration(showsCursor: false), "fresh owner starts audio-off")
        precondition(!sibling.streamConfiguration(showsCursor: false))
        first.handleSystemAudioCommand(Data("SNDOFF".utf8))
        precondition(first.stream?.configurations.isEmpty == true, "initial OFF is idempotent")
        first.handleSystemAudioCommand(Data("SNDON".utf8))
        precondition(first.stream?.configurations.last == true, "explicit ON enables the owner")
        precondition(sibling.stream?.configurations.last == false)
        let enabledUpdates = first.stream!.configurations.count
        sibling.handleSystemAudioCommand(Data("SNDON".utf8))
        precondition(first.stream!.configurations.count == enabledUpdates, "repeated ON avoids reconfiguration")
        sibling.handleSystemAudioCommand(Data("SNDOFF".utf8))
        precondition(first.stream?.configurations.last == false, "sibling OFF stops the shared plane")
        let disabledUpdates = first.stream!.configurations.count
        first.handleSystemAudioCommand(Data("SNDOFF".utf8))
        precondition(first.stream!.configurations.count == disabledUpdates, "repeated OFF avoids reconfiguration")
        first.handleSystemAudioCommand(Data("SNDON".utf8))
        ...
        sessions[3] = legacy
        precondition(!legacy.streamConfiguration(showsCursor: false), "last owner retirement clears audio opt-in")
        sibling.handleSystemAudioCommand(Data("SNDON".utf8))
        precondition(!legacy.streamConfiguration(showsCursor: false), "retired owner cannot re-enable a fresh lifetime")
        legacy.handleSystemAudioCommand(Data("SNDON".utf8))
        precondition(legacy.stream?.configurations.last == true, "fresh lifetime requires a new opt-in")
