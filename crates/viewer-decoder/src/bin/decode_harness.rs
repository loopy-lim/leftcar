...
fn minimal_stream() -> &'static [u8] {
    &[
        ...
        // IDR slice (nal 0x65), minimal
        0x00, 0x00, 0x00, 0x01, 0x65, 0x88, 0x84, 0x0a, 0x02, 0x80, 0x10, 0x1f, 0xc6, 0x18,
        0x80, 0x00, 0x03, 0x00, 0x80, 0x00, 0x1e, 0x07, 0x81, 0x74, 0x90, 0x24, 0x08, 0x48,
        0x20, 0x91, 0x04, 0x24, 0x08, 0x82, 0x04, 0x28, 0x82, 0x00, 0x00, 0x03, 0x00, 0x08,
        0x00, 0x00, 0x03, 0x00, 0x04, 0x1f, 0xc0, 0x00, 0x14, 0x1f, 0xe0, 0x00, 0x28, 0x3f,
        0xc0, 0x00, 0x50, 0x7f, 0x80, 0x00, 0xa0, 0xff, 0x00, 0x01, 0x41, 0xf0, 0x00, 0x02,
        0x83, 0xe0, 0x00, 0x05, 0x07, 0xc0, 0x00, 0x0a, 0x0f, 0x80, 0x00, 0x14, 0x1f, 0x00,
    ]
}
...
fn run(data: &[u8]) {
    ...
    if config.0.is_empty() || config.1.is_empty() || aus.is_empty() {
        eprintln!("BAD_STREAM config_sps={} config_pps={} aus={}", !config.0.is_empty(), !config.1.is_empty(), aus.len());
        std::process::exit(1);
    }
    println!("AUS {}", aus.len());

    unsafe {
        let mut decoder = match viewer_decoder::AndroidDecoder::new_h264(
            &config.0, &config.1, 64, 64, 0,
        ) {
            Ok(d) => d,
            Err(e) => {
        ...
        println!("SUMMARY frames_rendered={} of {} aus", decoder.frames_rendered, aus.len());
        let _ = rendered;
...
fn multi_decode(path: &str, count: usize) {
    ...
    unsafe {
        ...
        println!("SUMMARY instances={} frames_rendered={} of {} aus", count, sum, aus.len() * count);
