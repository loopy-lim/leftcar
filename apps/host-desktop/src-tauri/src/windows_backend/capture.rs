...
fn create_d3d_device() -> Result<(ID3D11Device, IDirect3DDevice), String> {
    ...
    let _context = context.ok_or("D3D11 returned no immediate context")?;
...
struct HardwareH264Encoder {
    ...
    // Keep the manager alive for the lifetime of the MFT input stream.
...
impl HardwareH264Encoder {
    fn new(
        ...
        let input_type = media_type(MFVideoFormat_ARGB32, width, height, fps, None)?;
        unsafe {
        ...
        unsafe { transform.SetInputType(0, &input_type, 0) }
            .map_err(win("set Media Foundation ARGB32 input type"))?;
        ...
        let dxgi_manager = dxgi_manager.ok_or("MFCreateDXGIDeviceManager returned null")?;
        unsafe { dxgi_manager.ResetDevice(device, reset_token) }
            .map_err(win("attach D3D11 device to DXGI manager"))?;
        unsafe {
            transform.ProcessMessage(MFT_MESSAGE_SET_D3D_MANAGER, dxgi_manager.as_raw() as usize)
        }
    ...
    fn encode(&mut self, texture: &ID3D11Texture2D) -> Result<Vec<Vec<u8>>, String> {
        ...
        let buffer = unsafe { MFCreateDXGISurfaceBuffer(&ID3D11Texture2D::IID, texture, 0, false) }
            .map_err(win("wrap WGC texture for Media Foundation"))?;
        let sample = unsafe { MFCreateSample() }.map_err(win("create Media Foundation sample"))?;
        unsafe { sample.AddBuffer(&buffer) }.map_err(win("attach ARGB32 texture sample buffer"))?;
        unsafe { sample.SetSampleTime(self.frame_index * self.frame_duration) }
            .map_err(win("set ARGB32 sample time"))?;
        unsafe { sample.SetSampleDuration(self.frame_duration) }
            .map_err(win("set ARGB32 sample duration"))?;
        unsafe { self.transform.ProcessInput(0, &sample, 0) }
            .map_err(win("submit texture sample to Media Foundation encoder"))?;
        self.frame_index += 1;
