...
interface SourceGrantEditorProps {
  deviceId: string;
  grants?: SourceGrantView;
  language: SupportedLanguage;
  onSaved: (deviceId: string, grants: SourceGrantView, startedAtEpoch?: number) => Promise<void>;
...
export default function SourceGrantEditor({
  deviceId,
  grants,
  language,
  onSaved,
  ...
  async function save(sourceIds: string[]): Promise<void> {
    setBusy(true);
    setError(null);
    try {
      ...
      const confirmed = await invoke<SourceGrantView>("set_source_grants", {
        deviceId,
        sourceIds,
        credentialId: grants.credentialId,
      });
      ...
      setUncertain(true);
      setError(String(cause));
      if (grants) onFailure?.(deviceId, grants.credentialId, String(cause));
    } finally {
      setBusy(false);
    }
  }
