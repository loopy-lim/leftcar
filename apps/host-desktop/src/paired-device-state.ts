...
export interface PairedDeviceState { revision: number; devices: PairedDevice[] }
...
let state: PairedDeviceState & { administrativeError: string | null } = {
  revision: -1, devices: [], administrativeError: null,
};
...
export const getPairedDeviceState = () => state;
export const subscribePairedDeviceState = (listener: () => void) => {
  listeners.add(listener); return () => { listeners.delete(listener); };
};
export const sourceGrantUncertaintyEpoch = (credentialId: string) => uncertain.get(credentialId)?.epoch ?? 0;
...
export function receivePairedDeviceState(snapshot: PairedDeviceState) {
  if (snapshot.revision < state.revision) return;
  const previousById = new Map(state.devices.map(device => [device.device_id, device]));
  const devices = snapshot.devices.filter(device => !removedCredentials.has(device.source_grants.credentialId)).map(device => {
    ...
    if (previous && previous.source_grants.stateRevision > snapshot.revision) return previous;
    const sameCredential = previous?.source_grants.credentialId === device.source_grants.credentialId;
    const grants = sameCredential && previous.source_grants.revision > device.source_grants.revision ? previous.source_grants : device.source_grants;
  ...
  for (const credential of uncertain.keys()) if (!current.has(credential)) uncertain.delete(credential);
  ...
  for (const [credential, revision] of removedCredentials) {
    if (snapshot.revision >= revision && !membership.has(credential)) removedCredentials.delete(credential);
  }
  publish({ ...state, ...snapshot, devices });
}
export function confirmSourceGrants(deviceId: string, grants: SourceGrantView, startedAtEpoch?: number) {
  const current = state.devices.find(device => device.device_id === deviceId);
  if (!current || current.source_grants.credentialId !== grants.credentialId || current.source_grants.revision > grants.revision) return;
  ...
  if (uncertain.get(grants.credentialId)?.epoch === startedAtEpoch) uncertain.delete(grants.credentialId);
  const confirmed = retainUncertainty({ ...grants, stateRevision: Math.max(grants.stateRevision, current.source_grants.stateRevision) });
  publish({ ...state, devices: state.devices.map(device => device === current ? { ...device, source_grants: confirmed } : device) });
}
export function markSourceGrantsUncertain(deviceId: string, credentialId: string, error: string) {
  const current = state.devices.find(device => device.device_id === deviceId && device.source_grants.credentialId === credentialId);
  if (!current) return;
  uncertain.set(credentialId, { epoch: ++failureEpoch, error });
  publish({ ...state, devices: state.devices.map(device => device === current ? { ...device, source_grants: retainUncertainty(device.source_grants) } : device) });
}
export function receiveRevokeOutcome(outcome: RevokeOutcome) {
  ...
  if (outcome.stateRevision > state.revision) {
    for (const removed of outcome.removedDevices) {
      removedCredentials.set(removed.credentialId, Math.max(outcome.stateRevision, removedCredentials.get(removed.credentialId) ?? -1));
