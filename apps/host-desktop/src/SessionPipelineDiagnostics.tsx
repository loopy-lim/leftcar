...
function Metric({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div className="inspector-item">
      ...
      <span className="inspector-item-value">{children}</span>
    </div>
  );
}
...
export default function SessionPipelineDiagnostics({
  ...
  return (
    <>
      ...
      {diagnostics.splitEncode !== null ? (
        <>
          <Metric label="타일 출력/표시">
            {diagnostics.splitEncode} · {diagnostics.splitRender}
          </Metric>
          <Metric label="분할 준비 / pair callback p95">
            {diagnostics.splitLatency}
          </Metric>
          <Metric label="분할 flow">{diagnostics.splitFlow}</Metric>
          <div className="inspector-item">
            <span className="inspector-item-label">분할 복구</span>
            <span className={cn(diagnosticValueVariants({
              tone: (session.splitPostEncodeDeltaDrops ?? 0) > 0
          ...
          <div className="inspector-item">
            <span className="inspector-item-label">타일 동기화</span>
            <span className={cn(diagnosticValueVariants({
              tone: (session.pairSyncTimeouts ?? 0) > 0 ? "warning" : "default",
            }))}>
              {diagnostics.splitSync} · 수신 손실 {diagnostics.splitLoss}
            </span>
          </div>
        </>
      ) : null}
      ...
      <Metric label="인코더 출력 간격 p95">
        {milliseconds(session.encodeOutputIntervalP95Us, "측정 중")}
      </Metric>
