...
> 2026-09-13 실행 기준: 사용자가 안정성 검사를 60분에서 **30분**으로 변경했다. 짧은 반복→10분→30분 순서이며 실기기는 사용자가 추후 직접 검사한다. 아래 과거 계획·예시의 60분과 수치 예시는 현재 실행 시간이나 관측 결과가 아니다. [현재 지원·수용 표](completion-and-support.md)를 우선한다.
...
## 17. 2026-09 감사 개선의 실행·판정 계약

이번 감사의 실기기 순서는 짧은 반복 측정, 10분 사전 검증, **30분 연속 안정성 검증**이다. 앞 절의 일반 예시나 과거 결과에 등장하는 60분을 이번 실행 시간으로 사용하지 않는다. 실제 진행 상태와 미검증 항목은 [감사 개선 검증 현황](2026-09-13-audit-remediation-validation.md)에 기록한다.

현재 수집기는 실행 환경 JSON에 선택한 ADB serial, Viewer package, Host PID, Host·Android·합성 소스 manifest 및 실제 적용 조건을 요구한다. 템플릿의 미확인 값을 요청 설정으로 채워 넣지 않는다.

```sh
bun tools/perf-matrix/collect.mjs \
  --context /absolute/path/run-context.json \
  --duration 1800 \
  --output /absolute/path/new-run
```

`collect-1440-4k.sh`도 같은 도구로 연결된다. 로그 버퍼를 지우거나 앱을 실행·재설정하지 않는다. 수집기는 자신이 만든 명령과 로그 프로세스만 종료한다. 실패·중단·프로세스 교체·시계 또는 로그 해석 오류에는 원시 로그와 가능한 실행 근거를 보존하고, 실패한 JSON 기록과 0이 아닌 종료값을 남긴다.

비교에는 같은 기기, 합성 소스 해시, 작업 종류, 연결 방식, 창 수, 실제 코덱·표시 모드, 소스·인코딩·Surface·물리 패널 크기와 주사율 및 같은 측정 시간이 필요하다. 실제 시간과 요청 시간, 양쪽 실행 시간의 허용 차이는 `min(1500ms, 요청 시간의 5%)`다. 준비·정리 시간을 측정 시간으로 바꾸어 부르지 않는다. 식별자나 시간 근거가 없는 과거 로그와 지원하지 않는 통계 형식은 비교 미지원으로 남긴다.

산출물 manifest schema 2는 **빌드한 컴퓨터**와 **산출물이 실행될 대상 OS·ABI**를 구분하고, 실제 ELF/Mach-O와 APK 안의 네이티브·JS/Hermes 바이트를 확인한다. 과거 schema 1 기록에 대상 검증 근거를 소급해서 붙이지 않는다. 빌드, 설치된 바이트, 실행 중인 프로세스, 영상 전달, 장시간 검증은 각각 별도 증거다.

프레임 카운터는 프로세스·스트림·수명·처리 단계별로 분리하며 카운터 초기화와 첫 샘플 이전의 누락을 보존한다. Host의 `ps` CPU 값과 RSS, Viewer의 원시 CPU tick 및 RSS는 측정 기준을 함께 남긴다. 온도는 현재 HAL과 캐시 값을 구분하고 온도가 아닌 센서 유형을 섭씨로 해석하지 않는다. 충전 중 배터리 값으로 소비 전력을 추정하지 않는다. 오디오·버퍼·광학 지연 근거가 없으면 미확인이다.
