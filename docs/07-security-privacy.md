...
- T-05 source capability(페어링 후 소스별 승인) 미구현 — 현재 토큰 보유자는
  모든 디스플레이 열람·스트림 시작 가능. 입력은 OS 입력 권한이 있으면 세션
  시작과 함께 자동으로 켜지며, 이 자동 활성화는 의도된 배포 설계다(§9의
  "소스 승인만으로 입력 capability 자동 획득 금지" 규칙과의 정합은 아직
  알려진 격차로 남는다). 소스별 승인은 이후 작업. 다음 배치 후보.
