...
- **E7 (계측된 장시간 실험)** — 검증 4의 30분 soak에서 지연·드롭·메모리 목표를 재현 가능하게 만족할 때 달성.
...
목표: USB 경로의 장시간 안정성을 확인한다. 이전 2026-08-26 설계의 60분 기준은 2026-09-13 사용자 결정에 따라 30분으로 변경했다. 실기기 검사는 사용자가 추후 직접 수행한다.

절차:

1. USB 스트리밍을 시작하고 30분간 유지한다.
...
합격 기준: 프레임 연속성 유지(30분간 세션 종료·재협상 없음), 제어 명령(`getStatus`) 응답 지연·손상 없음, 메모리 누수 없음.
