...
문서 상태: 현재 구현과 원래 제품 목표를 함께 보관
상태 기준일: 2026-09-13
현재 진입점: [지원 범위와 완료 기준](completion-and-support.md), [버전·업데이트 안내](versioning.md)
...
2. macOS 또는 Windows의 승인한 디스플레이를 Viewer의 별도 창으로 볼 수 있다. 앱 창 단독 캡처는 후속 목표다.
...
| 11 | [저지연 스트리밍 병목 조사](11-low-latency-investigation.md) | 현재 병목 가설을 어떻게 판정하고 무엇을 개선할 것인가 |
| 부록 | [Apple 화면 공유 비교 기준](apple-screen-sharing-baseline.md) | Apple UX·성능 기준과 Leftcar의 차이는 무엇인가 |
| 부록 | [macOS 지속 전체 화면 캡처](macos-persistent-capture.md) | 선택기 없는 VNC 캡처의 승인·서명·fallback 경계 |
...
| Leftcar Viewer | Android 휴대폰·태블릿·Galaxy XR에서 스트림을 디코딩하고 보여 주는 앱. 검증한 기기 조합은 지원 표 참조 |
| source | 현재 승인·캡처 가능한 디스플레이. 앱 창은 원래 도메인 모델의 후속 목표 |
...
- 현재 후보는 승인한 디스플레이 A, B, C를 각각 독립 source로 선택한다. 앱 창 단독 캡처는 구현되지 않았다.
...
아래는 초기 설계 승인 기록이다. 당시 비목표·전송 가설과 현재 구현의 차이는 제품 요구사항과 지원 표에 기록하며, 완료한 구현을 재승인 대기로 되돌리지 않는다.

- [x] 기본 UX가 Home Space의 멀티 인스턴스 stream window라는 점
- [x] 초기 승인: 입력 기본 거부·Host 스트림별 승인. 당시 제외했던 오디오·파일·클립보드는 이후 선택 기능으로 구현됨
