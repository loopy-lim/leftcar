...
- [ ] 기본 UX가 Home Space의 멀티 인스턴스 stream window라는 점
- [ ] 보기 전용이며 원격 입력, 오디오, 파일 전송을 제외한다는 점
- [ ] macOS를 첫 호스트로 삼고 Windows는 두 번째로 진행한다는 점
- [ ] 로컬 네트워크 직접 연결만 첫 배포 범위에 둔다는 점
- [ ] Rustra는 제어 계약에만 쓰고 영상은 네이티브 별도 경로로 보낸다는 점
- [ ] 전송 방식은 Galaxy XR 실기기 bake-off 결과로 결정한다는 점
- [ ] v1은 가상 디스플레이 드라이버를 만들지 않는다는 점
- [ ] 실제 Galaxy XR에서 같은 앱 창 4개를 동시에 열 수 있는지 Phase 1에서 검증한다는 점
