 ...
 */
export function invokeBatchSettled(entries) {
    const engine = runtime.engine;
    if (!engine) {
        if (isLazyConfigured())
            return ensureConfigured().then(() => invokeBatchSettled(entries));
    ...
    return (async () => {
        ...
        for (const entry of entries) {
            try {
                const value = await invokeWithTimeout(engine, entry.command, entry.args, entry.options);
        ...
        while (settled.length < entries.length)
            settled.push({ status: 'unexecuted' });
        return settled;
    })();
}
//# sourceMappingURL=global-batch-settled.js.map