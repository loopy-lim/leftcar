...
export function encodeUtf8(input) {
    if (nativeEncoder)
        return nativeEncoder.encode(input);
    let byteLength = 0;
...
export function decodeUtf8(input, start = 0, end = input instanceof Uint8Array ? input.length : input.byteLength) {
    const bytes = input instanceof Uint8Array ? input : new Uint8Array(input);
    if (nativeDecoder && start === 0 && end === bytes.length)
